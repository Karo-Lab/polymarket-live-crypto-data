use crate::{
    ...
    traits::{
        BoxStream, ColumnValue, ExchangeConnectorAdapter, TableSchema, WsError, WsReader,
        WsStream, WsWriter,
    },
};
...
impl<A: ExchangeConnectorAdapter> ExchangeConnector<A> {
    ...
    pub async fn run(mut self) {
        ...
        loop {
            ...
            match connect_ws(request).await {
                Ok((websocket_, _)) => {
                    backoff = 1u64;
...
fn target_host_port(uri: &tokio_tungstenite::tungstenite::http::Uri) -> Result<(String, u16), std::io::Error> {
    let host = uri
        .host()
        .ok_or_else(|| std::io::Error::new(std::io::ErrorKind::InvalidInput, "WebSocket URL missing host"))?;
...
async fn connect_ws(
    ...
    let base_stream: BoxStream = Box::new(TcpStream::connect((host.as_str(), port)).await?);

    client_async_tls_with_config(request, base_stream, None, None).await
}
